...
public class StudentCertificationAnswersUseCase {
    ...
    public CertificationStudentEntity execute(StudentCertificationAnswersDTO dto) throws Exception {
        ...
        var hasCertifivcation = this.verifyIfHasCertificationUseCase.execute(new VerifyHasCertifivcationDTO(dto.getEmail(), dto.getTechnology()));
        if (hasCertifivcation) {
        ...
        AtomicInteger correctAnsers = new AtomicInteger(0);
        ...
        dto.getQuestionsAnswers().stream().forEach(questionAnswer -> {
            ...
            if (findCorrectAlternative.getId().equals(questionAnswer.getAlternativeID())) {
                ...
                correctAnsers.incrementAndGet();
            } else {
                questionAnswer.setCorrect(false);
            }

            var answerCertificationEntity = AnswersCertificationEntity.builder().answerID(questionAnswer.getAlternativeID()).questionID(questionAnswer.getQuestionID())
        ...
        CertificationStudentEntity certificationStudentEntity = CertificationStudentEntity.builder()
            .technology(dto.getTechnology()).studentID(studentID).grade(correctAnsers.get()).build();
